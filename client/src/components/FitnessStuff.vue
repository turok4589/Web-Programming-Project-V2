           ...
           <div class = "column is-two-thirds">
                  ...
                  <div class="field" v-if="updatexercise == 1">
                    <h2><strong>Update Your Exercises With The Following Fields</strong></h2>
                      ...
                      <div class="control">
                          <input class="input" type="Number" placeholder="Weight" v-model="exercise_id">
                      </div>
                  </div>
                   ...
                   <div class="field" v-if="updatexercise == 1 && (exercise_Type_id == 2 || exercise_Type_id == 3 || exercise_Type_id == 4 || exercise_Type_id == 5)">
                     <label class="label"> <strong>You selected: {{ options[exercise_Type_id - 1].text}}</strong></label>
                     <div class="control">
                        <label class="label"> <strong>Enter the distance you travelled:</strong></label>
                         <input class="input" type="Number" placeholder="Miles Ran" v-model="distance">
                    ...
                    <div class = "field" v-if="updatexercise == 1 && (exercise_Type_id == 1 || exercise_Type_id == 6)"> 
                        <label class="label"> <strong>You selected: {{ options[exercise_Type_id - 1].text}}</strong></label>
                          <div class="control">
                            <label class="label"> <strong>Enter the amount of weight you lifted:</strong></label>
                              <input class="input" type="Number" placeholder="Weight" v-model="weight">
                   ...
                   <div class="control">
                    ...
                    <div class="field" v-if="deletexercise == 1">
                       <h2><strong>Update Your Exercises With The Following Fields</strong></h2>
                        <label class="label"> <strong>Enter Exercise ID:</strong></label>
                        <div class="control">
                            <input class="input" type="Number" placeholder="Exercise ID:" v-model="exercise_id">
                        </div>
                    </div>
...
<script>
import { addexercise, comment, getexercisetypes, getListForUser, updatexercise } from "../models/exercise";
...
export default {
     ...
     methods: {
        ...
        async addexercise(){
          const user_id = session.user_id;
          calories_Burned =  this.calculatecalories(exercise_types[exercise_Type_id - 1].MET_Score, this.bodyweight, this.time_Spent);
          const response = await addexercise(this.time_Spent, this.calories_Burned, this.favorite_Exercise, this.exercise_Type_id, this.distance, this.exercise_Sets, this.reps_Per_Set, this.lifting_Weight, this.bodyweight, user_id);
          this.fitnessstuff.Exercises.push(response);
        },
        async updatexercisetable(){
          const user_id = session.user_id;
          calories_Burned =  this.calculatecalories(exercise_types[exercise_Type_id - 1].MET_Score, this.bodyweight, this.time_Spent);
          const response = await updatexercise(this.exercise_id, this.time_Spent, this.calories_Burned, this.favorite_Exercise, this.exercise_Type_id, this.distance, this.exercise_Sets, this.reps_Per_Set, this.lifting_Weight, this.bodyweight, user_id);
          this.updatexercise = 0;
        },
        async deleteexerciserow(){
           const user_id = session.user_id;
           const response = await deleteexercise(this.exercise_id, user_id);
